...
use crate::lang::value::{LoweredList, LoweredType, Type, Value, exact_i64_to_f64, format_float};
...
impl Codec {
    ...
    /// Returns whether this codec accepts `ty`.
    /// Optionals are rejected until flow refinement proves an `@if-present` guard.
    pub fn accepts_type(self, ty: &Type) -> bool {
        if ty.is_optional() {
            return false;
        }
        let inner = ty.lowered_type();
        match self {
            Self::Text => matches!(
                inner,
                LoweredType::String | LoweredType::Path | LoweredType::Enum(_)
            ),
            Self::Int => matches!(inner, LoweredType::Int),
            Self::Float => matches!(inner, LoweredType::Float | LoweredType::Int),
            Self::Bool => matches!(inner, LoweredType::Bool),
            Self::TomlArray => match inner {
                LoweredType::List(LoweredList::Homogeneous(item)) => is_toml_scalar_type(item),
                LoweredType::List(LoweredList::Tuple(types)) => {
                    types.iter().all(is_toml_scalar_type)
                }
                _ => false,
            },
            Self::TomlString
    ...
    /// Encodes a typed value with this codec.
    pub fn encode(self, value: &Value) -> Result<String, String> {
        if value.is_null() {
            return Err("value is #null; guard the reference with `@if-present`".to_owned());
        }
        match self {
...
/// Encodes a TOML basic string, including quotes and escapes.
...
/// Quotes a string as one POSIX shell word using single quotes.
...
/// and `{{path:codec}}` but rejects codec-before-path forms.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
    ...
    while !rest.is_empty() {
        match rest.find("{{") {
            ...
            Some(open) => {
                ...
                // A closing delimiter inside a quoted literal belongs to the
                // literal payload rather than the directive.
...
pub fn render_template_with(
    ...
    render_template_with_limit(input, syntax, lookup, u64::MAX)
}

/// Render a template while rejecting each append that would exceed
/// `max_bytes`. The check happens before mutating the output buffer.
pub fn render_template_with_limit(
    ...
    let capacity = usize::try_from(max_bytes).map_or(input.len(), |limit| input.len().min(limit));
    let mut out = String::with_capacity(capacity);
    for segment in segments {
        match segment {
            Segment::Text(text) => append_with_limit(&mut out, text, max_bytes)?,
            Segment::Directive { parsed, raw } => match parsed {
                Directive::Literal(text) => append_with_limit(&mut out, &text, max_bytes)?,
                Directive::Substitute { codec, name } => {
                    ...
                    append_with_limit(&mut out, &encoded, max_bytes)?;
                }
            },
        }
    }
...
fn append_with_limit(out: &mut String, value: &str, max_bytes: u64) -> Result<(), String> {
    let projected = u64::try_from(out.len())
        .ok()
        .and_then(|len| len.checked_add(u64::try_from(value.len()).ok()?));
    if projected.is_none_or(|len| len > max_bytes) {
        return Err(format!(
            "rendered template exceeds the maximum of {max_bytes} bytes"
        ));
    }
    out.push_str(value);
    Ok(())
...
mod tests {
    ...
    fn null_needs_guard() {
        ...
        assert!(err.contains("@if-present"), "{err}");
    }
    ...
    #[test]
    fn bounded_render_checks_each_append_without_changing_in_limit_bytes() {
        let lookup = |_: &str| Some(Value::String("cd".to_owned()));
        assert_eq!(
            render_template_with_limit("ab{{value}}ef", TemplateSyntax::V3, &lookup, 6).unwrap(),
            "abcdef"
        );
        let error = render_template_with_limit("ab{{value}}ef", TemplateSyntax::V3, &lookup, 5)
            .unwrap_err();
        assert!(error.contains("maximum of 5 bytes"), "{error}");
    }
}
