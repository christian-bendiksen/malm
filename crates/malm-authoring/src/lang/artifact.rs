...
#[derive(Debug)]
pub struct Artifact {
    /// Destination as written in `to=`, relative to the target root or `~/`.
    pub to: String,
    pub content: ArtifactContent,
    pub executable: bool,
    /// Local format used for pure structural checks.
    pub format: String,
    /// Explicit component transforms in authoring declaration order.
...
/// Generated bytes or a canonical document deferred to the engine's component host.
#[derive(Debug)]
...
pub(crate) fn format_known(name: &str) -> bool {
    KNOWN_FORMATS.contains(&name)
}

pub(crate) fn known_formats_help() -> String {
    format!("known formats: {}", KNOWN_FORMATS.join(", "))
}
...
pub fn validate_format(name: &str, content: &str) -> Vec<String> {
    match name {
        ...
        "toml" => deep_syntax_deferred(),
        "xml" => deep_syntax_deferred(),
        "ini" => validate_ini(content),
        "kdl-v1" => validate_kdl(content, KdlDialect::V1),
        "kdl-v2" => validate_kdl(content, KdlDialect::V2),
        // Hypr-style configuration accepts comments, blank lines, assignments,
        // and `section {` ... `}` blocks.
        ...
        other => vec![format!("unknown format `{other}`")],
    }
}

/// Returns no findings for formats without a local structural validator.
/// Authors can request additional processing through a component transform.
fn deep_syntax_deferred() -> Vec<String> {
    Vec::new()
}

fn validate_shell(_content: &str) -> Vec<String> {
    deep_syntax_deferred()
}

fn validate_lua(_content: &str) -> Vec<String> {
    deep_syntax_deferred()
}

fn validate_css(_content: &str) -> Vec<String> {
    deep_syntax_deferred()
}

fn validate_gtk_css(content: &str) -> Vec<String> {
    // Reject unsafe control characters without attempting a full CSS parse.
    if let Err(problem) = mask_gtk_css(content) {
        return vec![format!("invalid GTK CSS: {problem}")];
    }
    deep_syntax_deferred()
}
...
/// Validates Hyprland-style `key = value` assignments (the key may be
/// dotted or colon-namespaced), `name {` block openers, bare `}` closers,
/// comments, blank lines, and brace balance.
...
fn strip_json_comments(input: &str) -> Result<String, String> {
    ...
    while i < bytes.len() {
        ...
        match c {
            ...
            // Remove a trailing comma while preserving the following whitespace.
            ',' => {
                let mut j = i + 1;
...
"#;
    ...
    #[test]
    fn gtk_css_rejects_unsafe_input() {
        let problems = validate_format("gtk-css", "label { color: red; }\0");
        assert!(!problems.is_empty(), "control character was accepted");
    }
}
